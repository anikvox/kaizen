...
// Separate app for focus SSE (supports both device token and Clerk auth via query param)
export const focusSSE = new Hono();

focusSSE.get("/", async (c) => {
  ...
  return streamSSE(c, async (stream) => {
    ...
    const cleanup = events.onFocusChanged(async (data) => {
      // Only send if this user's focus changed
